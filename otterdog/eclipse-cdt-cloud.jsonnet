...
orgs.newOrg('eclipse-cdt-cloud') {
  ...
  _repositories+:: [
    ...
    orgs.newRepo('trace-viewer-examples') {
      ...
      delete_branch_on_merge: false,
      dependabot_security_updates_enabled: true,
      description: "Trace viewer example applications and building blocks that can be used in or with them. Each application integrates either the Eclipse Theia trace viewer extension or the trace viewer extension for VS Code",
      homepage: "",
      topics+: [
        "trace-viewer",
        "theia-traceviewer",
